...
public class MQTTService extends Service implements CallbackMQTTClient.IMQTTMessageReceiver {
    ...
    private static final String TAG = "sdses_MQTTService";
    ...
    public void OnCreate(AppCompatActivity appCompatActivity) {
        ...
        android.util.Log.d(TAG, "MQTTService 读取本地存储的Setting和Tab信息，初始化组件");
        // 读取本地存储的Setting和Tab信息
    ...
    public MQTTService() {
        ...
        Log.d(TAG, "MQTTService constructor MQTTService()");
    ...
    public void onCreate() {
        super.onCreate();
        Log.d(TAG, "MQTTService onCreate()，启动服务....");
    }

    @Override
    public void onDestroy() {
        super.onDestroy();
        Log.d(TAG, "MQTTService onDestroy()，服务销毁...");
    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (callbackMQTTClient == null) {
            Log.d(TAG, "new CallbackMQTTClient()");
            callbackMQTTClient = new CallbackMQTTClient(this);
        }

        //callbackMQTTClient.reConnect(appSettings);

        Log.d(TAG, "clientCountsInForeground = " + clientCountsInForeground);
    ...
    public void onReceiveMQTTMessage(String topic, Buffer payload) {
        if (topic.equals(SERVER_DATAPACK_NAME)) {
            ...
            try {
                ...
                android.util.Log.d(TAG, "onReceiveMQTTMessage 收到的消息 : " + payloadAsString);
            ...
            // 处理得到的json信息
//            HandMessage.parseChatJson(payloadAsString);
            HandMessage.parseChatJsonPlus(payloadAsString, getApplicationContext());

            /*processReceiveSimplyTopicPayloadData(topic, payloadAsString);*/ // TODO 消息通知
        }
    }
